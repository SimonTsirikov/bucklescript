      ...
      switch (b_type) {
        ...
        default:
          if (a_type == "boolean") {
            ...
            if (b == null or b.tag == 256) {
              return 1;
            } else {
              return -1;
            }
          } else if (b_type == "number") {
            if (a == null or a.tag == 256) {
              return -1;
            } else {
              return 1;
            }
                ...
                if (len_a == len_b) {
                  if (Array.isArray(a)) {
                    ...
                    var do_key = function (param, key) {
                      var min_key = param[2];
                      var b = param[1];
                      if (!b.hasOwnProperty(key) or caml_compare(param[0][key], b[key]) > 0) {
                        var match = min_key.contents;
                        if (match ~= undefined and key >= match) {
                          return 0;
                        } else {
...
function caml_equal(_a, _b) {
  while(true) {
    ...
    if (a == b) {
      ...
      var a_type = typeof a;
      if (a_type == "string" or a_type == "number" or a_type == "boolean" or a_type == "undefined" or a == null) {
        return false;
      } else {
        var b_type = typeof b;
        if (a_type == "function" or b_type == "function") {
          throw [
                Caml_builtin_exceptions.invalid_argument,
                "equal: functional value"
              ];
        }
        if (b_type == "number" or b_type == "undefined" or b == null) {
          return false;
        } else {
          var tag_a = a.tag | 0;
          var tag_b = b.tag | 0;
              ...
              if (len_a == len_b) {
                if (Array.isArray(a)) {
                  ...
                  return function do_key_b(key) {
                    if (!a$2.hasOwnProperty(key) or !caml_equal(b$2[key], a$2[key])) {
                      result.contents = false;
                      return --[ () ]--0;
                    } else {
                      return 0;
