...
function break_new_line(state, offset, width) do
  ...
  var real_indent = state.pp_max_indent < indent and state.pp_max_indent or indent;
...
function pp_force_break_line(state) do
  ...
  if (match) then do
    var match$1 = match[0];
    var width = match$1[1];
    if (width > state.pp_space_left and (match$1[0] - 1 >>> 0) <= 3) then do
    ...
    do
       ...
       if ___conditional___ = 1--[ Pp_break ]-- then do
          ...
          if (match$6) then do
            ...
            var width = match$7[1];
            local ___conditional___=(match$7[0]);
            do
               ...
               if ___conditional___ = 4--[ Pp_box ]-- then do
                  if (state.pp_is_new_line or !(size > state.pp_space_left or state.pp_current_indent > ((state.pp_margin - width | 0) + off | 0))) then do
                    return break_same_line(state, n);
                  end else do
                    return break_new_line(state, off, width);
                  end end end end end 
       ...
       if ___conditional___ = 3--[ Pp_begin ]-- then do
          ...
          if (insertion_point$1 > state.pp_max_indent) then do
            pp_force_break_line(state);
          end
           end 
          var offset$1 = state.pp_space_left - param[0] | 0;
          var bl_type = ty ~= 1 and (
...
function advance_left(state) do
  try do
    ...
    while(true) do
      ...
      if (size < 0 and (state$1.pp_right_total - state$1.pp_left_total | 0) < state$1.pp_space_left) then do
        return 0;
      end else do
...
function set_size(state, ty) do
  ...
  if (match) then do
    ...
    var t = match[1];
    if (match$1[0] < state.pp_left_total) then do
...
function pp_set_margin(state, n) do
  if (n >= 1) then do
    var n$1 = pp_limit(n);
    state.pp_margin = n$1;
    var new_max_indent = state.pp_max_indent <= state.pp_margin and state.pp_max_indent or Caml_primitive.caml_int_max(Caml_primitive.caml_int_max(state.pp_margin - state.pp_min_space_left | 0, state.pp_margin / 2 | 0), 1);
...
function pp_set_formatter_out_channel(state, oc) do
  state.pp_out_string = (function (param, param$1, param$2) do
...
function make_formatter(output, flush) do
  var ppf = pp_make_formatter(output, flush, (function (prim) do
          return --[ () ]--0;
        end), (function (prim) do
          return --[ () ]--0;
        end), (function (prim) do
          return --[ () ]--0;
        end));
...
function formatter_of_symbolic_output_buffer(sob) do
  var f = function (param, param$1, param$2) do
    var sob$1 = sob;
    var s = param;
    var i = param$1;
    var n = param$2;
...
function print_as(param, param$1) do
...
function print_break(param, param$1) do
...
function print_tbreak(param, param$1) do
...
function set_formatter_output_functions(param, param$1) do
...
function pp_print_list(_$staropt$star, pp_v, ppf, _param) do
  while(true) do
    ...
    var pp_sep = $staropt$star ~= undefined and $staropt$star or pp_print_cut;
    if (param) then do
      var vs = param[1];
      var v = param[0];
      if (vs) then do
...
function output_acc(ppf, acc) do
  ...
  if (typeof acc == "number") then do
    ...
    do
       ...
       if ___conditional___ = 2--[ Acc_string_literal ]-- then do
          var p$3 = acc[0];
          if (typeof p$3 ~= "number" and !p$3.tag) then do
            var match$2 = p$3[1];
            if (typeof match$2 ~= "number" and match$2.tag == --[ Magic_size ]--1) then do
              p = p$3[0];
              size = match$2[1];
              s = acc[1];
              exit = 1;
       ...
       if ___conditional___ = 3--[ Acc_char_literal ]-- then do
          var p$4 = acc[0];
          if (typeof p$4 ~= "number" and !p$4.tag) then do
            var match$3 = p$4[1];
            if (typeof match$3 ~= "number" and match$3.tag == --[ Magic_size ]--1) then do
              p$1 = p$4[0];
              size$1 = match$3[1];
              c = acc[1];
              exit = 2;
       ...
       if ___conditional___ = 4--[ Acc_data_string ]-- then do
          var p$5 = acc[0];
          if (typeof p$5 ~= "number" and !p$5.tag) then do
            var match$4 = p$5[1];
            if (typeof match$4 ~= "number" and match$4.tag == --[ Magic_size ]--1) then do
              p = p$5[0];
              size = match$4[1];
              s = acc[1];
              exit = 1;
       ...
       if ___conditional___ = 5--[ Acc_data_char ]-- then do
          var p$6 = acc[0];
          if (typeof p$6 ~= "number" and !p$6.tag) then do
            var match$5 = p$6[1];
            if (typeof match$5 ~= "number" and match$5.tag == --[ Magic_size ]--1) then do
              p$1 = p$6[0];
              size$1 = match$5[1];
              c = acc[1];
              exit = 2;
  ...
  do
     ...
     if ___conditional___ = 2 then do
        output_acc(ppf, p$1);
        return pp_print_as_size(ppf, size$1, Caml_bytes.bytes_to_string(Bytes.make(1, c)));end end end 
     do
...
function strput_acc(ppf, acc) do
  ...
  if (typeof acc == "number") then do
    ...
    do
       ...
       if ___conditional___ = 6--[ Acc_delay ]-- then do
          var p$7 = acc[0];
          if (typeof p$7 ~= "number" and !p$7.tag) then do
            var match$6 = p$7[1];
            if (typeof match$6 ~= "number" and match$6.tag == --[ Magic_size ]--1) then do
