...
function peek_char(ib) do
  if (ib.ic_current_char_is_valid) do
...
function token_int_literal(conv, ib) do
  ...
  var l = #tok;
  if (l == 0 or Caml_string.get(tok, 0) ~= --[ "+" ]--43) do
    return tok;
  end else do
    ...
    if (Curry._1(digitp, c)) do
      ...
      while(true) do
        ...
        if (width$3 == 0) do
          ...
          if (ib$2.ic_eof) do
            return width$3;
          end else if (Curry._1(digitp$1, c$1)) do
            var width$4 = store_char(width$3, ib$2, c$1);
            _width = width$4;
            continue ;
          end else if (c$1 ~= 95) do
            return width$3;
          end else do
...
function is_hexa_digit(param) do
  ...
  if (switcher > 22 or switcher < 0) do
...
function scan_fractional_part(width, ib) do
  if (width == 0) do
    ...
    var c = peek_char(ib);
    if (ib.ic_eof or c > 57 or c < 48) do
      return width;
    end else do
...
function scan_exponent_part(width, ib) do
  if (width == 0) do
    ...
    var c = peek_char(ib);
    if (ib.ic_eof or c ~= 69 and c ~= 101) do
      return width;
    end else do
...
function check_case_insensitive_string(width, ib, error, str) do
  ...
  for(var i = 0 ,i_finish = len - 1 | 0; i <= i_finish; ++i)do
    var c = peek_char(ib);
    if (lowercase(c) ~= lowercase(Caml_string.get(str, i))) do
      Curry._1(error, --[ () ]--0);
    end
    if (width$1 == 0) do
      Curry._1(error, --[ () ]--0);
...
function scan_hex_float(width, precision, ib) do
  if (width == 0 or end_of_input(ib)) do
    throw [
          Scan_failure,
          "not a valid float in hexadecimal notation"
        ];
  ...
  var width$1 = scan_sign(width, ib);
  if (width$1 == 0 or end_of_input(ib)) do
    throw [
          Scan_failure,
          "not a valid float in hexadecimal notation"
        ];
  ...
  if (c >= 78) do
    var switcher = c - 79 | 0;
    if (switcher > 30 or switcher < 0) do
      if (switcher >= 32) do
        throw [
              Scan_failure,
              "not a valid float in hexadecimal notation"
        ...
        var width$2 = store_char(width$1, ib, c);
        if (width$2 == 0 or end_of_input(ib)) do
          throw [
                Scan_failure,
                "not a valid float in hexadecimal notation"
              ];
    ...
    if (width$3 == 0 or end_of_input(ib)) do
      throw [
            Scan_failure,
            "not a valid float in hexadecimal notation"
          ];
    ...
    if (width$4 == 0 or end_of_input(ib)) do
      return width$4;
    end else do
      ...
      if (width$5 == 0 or end_of_input(ib)) do
        return width$5;
      end else do
        ...
        if (c$1 ~= 46) do
          ...
          if (width$7 == 0 or end_of_input(ib)) do
            width$6 = width$7;
          end else do
            var match$1 = peek_char(ib);
            if (match$1 ~= 80 and match$1 ~= 112) do
        ...
        if (width$6 == 0 or end_of_input(ib)) do
          return width$6;
        end else do
          var c$2 = peek_char(ib);
          var exit = 0;
          if (c$2 ~= 80 and c$2 ~= 112) do
            return width$6;
          end else do
            exit = 2;
          end
          if (exit == 2) do
            ...
            if (width$8 == 0 or end_of_input(ib)) do
              throw [
                    Scan_failure,
                    "not a valid float in hexadecimal notation"
                  ];
  ...
  if (width$9 == 0 or end_of_input(ib)) do
    throw [
          Scan_failure,
          "not a valid float in hexadecimal notation"
        ];
...
function scan_caml_float_rest(width, precision, ib) do
  ...
  var c = peek_char(ib);
  var switcher = c - 69 | 0;
  if (switcher > 32 or switcher < 0) do
    if (switcher ~= -23) do
      throw [
            Scan_failure,
            "no dot or exponent part found in float token"
  ...
  end else if (switcher > 31 or switcher < 1) do
      ...
      switch (c) do
        ...
        case 120 :
            var get_digit = function (param) do
              ...
              var switcher = c - 48 | 0;
              if (switcher > 22 or switcher < 0) do
                if (switcher > 54 or switcher < 49) do
                  return bad_input_escape(c);
                end else do
                  return c;
                end
              end else if (switcher > 16 or switcher < 10) do
                return c;
              end else do
                return bad_input_escape(c);
              end
...
function scan_chars_in_char_set(char_set, scan_indic, width, ib) do
  var scan_chars = function (_i, stp) do
    while(true) do
      var i = _i;
      var c = peek_char(ib);
      if (i > 0 and !ib.ic_eof and CamlinternalFormat.is_in_char_set(char_set, c) and c ~= stp) do
...
function scanf_bad_input(ib, x) do
  var s;
  if (x[0] == Scan_failure or x[0] == Caml_builtin_exceptions.failure) do
    s = x[1];
  end else do
    throw x;
  end
...
function stopper_of_formatting_lit(fmting) do
  if (fmting == --[ Escaped_percent ]--6) do
    return --[ tuple ]--[
            --[ "%" ]--37,
            ""
          ];
    ...
    if (typeof fmt == "number") do
      ...
      switch (fmt.tag | 0) do
        ...
        case --[ Scan_char_set ]--20 :
            ...
            var char_set = fmt[1];
            var width_opt = fmt[0];
            if (typeof rest$1 ~= "number" and rest$1.tag == --[ Formatting_lit ]--17) do
...
function kscanf(ib, ef, param) do
  ...
  var k = function (readers, f) do
    ...
    try do
      ...
      if (exc[0] == Scan_failure or exc[0] == Caml_builtin_exceptions.failure or exc == Caml_builtin_exceptions.end_of_file) do
        match = --[ Exc ]--Block.__(1, [exc]);
      end else if (exc[0] == Caml_builtin_exceptions.invalid_argument) do
