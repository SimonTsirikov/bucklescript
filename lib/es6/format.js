...
function pp_force_break_line(state) {
  ...
  if (match) {
    var match$1 = match[0];
    var width = match$1[1];
    if (width > state.pp_space_left and (match$1[0] - 1 >>> 0) <= 3) {
      ...
      case --[ Pp_break ]--1 :
          ...
          if (match$6) {
            ...
            switch (match$7[0]) {
              ...
              case --[ Pp_box ]--4 :
                  if (state.pp_is_new_line or !(size > state.pp_space_left or state.pp_current_indent > ((state.pp_margin - width | 0) + off | 0))) {
                    return break_same_line(state, n);
                  } else {
                    return break_new_line(state, off, width);
                  }
...
function advance_left(state) {
  try {
    ...
    while(true) {
      ...
      if (size < 0 and (state$1.pp_right_total - state$1.pp_left_total | 0) < state$1.pp_space_left) {
        return 0;
      } else {
...
function pp_close_tbox(state, param) {
  if (state.pp_curr_depth > 1 and state.pp_curr_depth < state.pp_max_boxes) {
    var elem = {
      elem_size: 0,
      token: --[ Pp_tend ]--2,
      length: 0
  ...
  if (typeof acc == "number") {
    ...
    switch (acc.tag | 0) {
      ...
      case --[ Acc_string_literal ]--2 :
          var p$3 = acc[0];
          if (typeof p$3 ~= "number" and !p$3.tag) {
            var match$2 = p$3[1];
            if (typeof match$2 ~= "number" and match$2.tag == --[ Magic_size ]--1) {
              p = p$3[0];
              size = match$2[1];
              s = acc[1];
              exit = 1;
      ...
      case --[ Acc_char_literal ]--3 :
          var p$4 = acc[0];
          if (typeof p$4 ~= "number" and !p$4.tag) {
            var match$3 = p$4[1];
            if (typeof match$3 ~= "number" and match$3.tag == --[ Magic_size ]--1) {
              p$1 = p$4[0];
              size$1 = match$3[1];
              c = acc[1];
              exit = 2;
      ...
      case --[ Acc_data_string ]--4 :
          var p$5 = acc[0];
          if (typeof p$5 ~= "number" and !p$5.tag) {
            var match$4 = p$5[1];
            if (typeof match$4 ~= "number" and match$4.tag == --[ Magic_size ]--1) {
              p = p$5[0];
              size = match$4[1];
              s = acc[1];
              exit = 1;
      ...
      case --[ Acc_data_char ]--5 :
          var p$6 = acc[0];
          if (typeof p$6 ~= "number" and !p$6.tag) {
            var match$5 = p$6[1];
            if (typeof match$5 ~= "number" and match$5.tag == --[ Magic_size ]--1) {
              p$1 = p$6[0];
              size$1 = match$5[1];
              c = acc[1];
              exit = 2;
...
function strput_acc(ppf, acc) {
  ...
  if (typeof acc == "number") {
    ...
    switch (acc.tag | 0) {
      ...
      case --[ Acc_delay ]--6 :
          var p$7 = acc[0];
          if (typeof p$7 ~= "number" and !p$7.tag) {
            var match$6 = p$7[1];
            if (typeof match$6 ~= "number" and match$6.tag == --[ Magic_size ]--1) {
