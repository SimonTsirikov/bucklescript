...
let requires require_lit cxt f (modules : (Ident.t * string) list ) =
  ...
  Ext_list.rev_iter reversed_list (fun (s,file) ->
      ...
      P.space f;
      P.string f "\"";
      P.string f ( String.concat "." (List.filter (fun x -> x <> "bs-platform") (String.split_on_char '/' (String.sub file 0 (if String.length file > 4 then String.length file - 4 else String.length file)))));
      P.string f "\"";
      P.string f L.semi;
      P.newline f ;
    ) ;
