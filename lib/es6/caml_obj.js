...
function caml_obj_truncate(x, new_size) do
  ...
  if (len ~= new_size) then do
    for var i = new_size , len - 1 | 0 , 1 do
      x[i] = 0;
    end
    x.length = new_size;
    return --[ () ]--0;
                ...
                if (len_a == len_b) then do
                  if (Array.isArray(a)) then do
                    ...
                    var do_key = function (param, key) do
                      var min_key = param[2];
                      var b = param[1];
                      if (!b.hasOwnProperty(key) or caml_compare(param[0][key], b[key]) > 0) then do
              ...
              if (len_a == len_b) then do
                if (Array.isArray(a)) then do
                  ...
                  return function do_key_b(key) do
                    if (!a$2.hasOwnProperty(key) or !caml_equal(b$2[key], a$2[key])) then do
                      result.contents = false;
                      return --[ () ]--0;
                    end else do
                      return 0;
