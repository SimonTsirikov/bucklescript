...
function token_int_literal(conv, ib) {
  ...
  var l = #tok;
  if (l == 0 or Caml_string.get(tok, 0) ~= --[ "+" ]--43) {
    return tok;
  } else {
...
function scan_exponent_part(width, ib) {
  if (width == 0) {
    ...
    var c = peek_char(ib);
    if (ib.ic_eof or c ~= 69 and c ~= 101) {
      return width;
    } else {
...
function scan_hex_float(width, precision, ib) {
  ...
  var width$1 = scan_sign(width, ib);
  if (width$1 == 0 or end_of_input(ib)) {
    throw [
          Scan_failure,
          "not a valid float in hexadecimal notation"
        ];
  ...
  if (c >= 78) {
    ...
    if (switcher > 30 or switcher < 0) {
      if (switcher >= 32) {
        ...
        var width$2 = store_char(width$1, ib, c);
        if (width$2 == 0 or end_of_input(ib)) {
          throw [
                Scan_failure,
                "not a valid float in hexadecimal notation"
              ];
    ...
    if (width$3 == 0 or end_of_input(ib)) {
      throw [
            Scan_failure,
            "not a valid float in hexadecimal notation"
          ];
    ...
    if (width$4 == 0 or end_of_input(ib)) {
      return width$4;
    } else {
      ...
      if (width$5 == 0 or end_of_input(ib)) {
        return width$5;
      } else {
        ...
        if (c$1 ~= 46) {
          ...
          if (width$7 == 0 or end_of_input(ib)) {
            width$6 = width$7;
          } else {
            var match$1 = peek_char(ib);
            if (match$1 ~= 80 and match$1 ~= 112) {
        ...
        if (width$6 == 0 or end_of_input(ib)) {
          return width$6;
        } else {
          var c$2 = peek_char(ib);
          var exit = 0;
          if (c$2 ~= 80 and c$2 ~= 112) {
            return width$6;
          } else {
            exit = 2;
          }
          if (exit == 2) {
            ...
            if (width$8 == 0 or end_of_input(ib)) {
              throw [
                    Scan_failure,
                    "not a valid float in hexadecimal notation"
                  ];
  ...
  if (width$9 == 0 or end_of_input(ib)) {
    throw [
          Scan_failure,
          "not a valid float in hexadecimal notation"
        ];
...
function scan_caml_float_rest(width, precision, ib) {
  ...
  } else if (switcher > 31 or switcher < 1) {
      ...
      switch (c) {
        ...
        case 120 :
            var get_digit = function (param) {
              ...
              var switcher = c - 48 | 0;
              if (switcher > 22 or switcher < 0) {
                if (switcher > 54 or switcher < 49) {
                  return bad_input_escape(c);
                } else {
                  return c;
                }
              } else if (switcher > 16 or switcher < 10) {
                return c;
              } else {
                return bad_input_escape(c);
              }
...
function scan_chars_in_char_set(char_set, scan_indic, width, ib) {
  var scan_chars = function (_i, stp) {
    while(true) {
      var i = _i;
      var c = peek_char(ib);
      if (i > 0 and !ib.ic_eof and CamlinternalFormat.is_in_char_set(char_set, c) and c ~= stp) {
    ...
    if (typeof fmt == "number") {
      ...
      switch (fmt.tag | 0) {
        ...
        case --[ Scan_char_set ]--20 :
            ...
            var char_set = fmt[1];
            var width_opt = fmt[0];
            if (typeof rest$1 ~= "number" and rest$1.tag == --[ Formatting_lit ]--17) {
...
function kscanf(ib, ef, param) {
  ...
  var k = function (readers, f) {
    ...
    try {
      ...
      if (exc[0] == Scan_failure or exc[0] == Caml_builtin_exceptions.failure or exc == Caml_builtin_exceptions.end_of_file) {
