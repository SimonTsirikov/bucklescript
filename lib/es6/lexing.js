...
function from_function(f) do
  ...
  return do
          refill_buff: (function (param) do
              ...
              var lexbuf = param;
              var read = Curry._2(read_fun, aux_buffer, #aux_buffer);
              var n = read > 0 ? read : (lexbuf.lex_eof_reached = true, 0);
              if ((lexbuf.lex_buffer_len + n | 0) > #lexbuf.lex_buffer) do
                if (((lexbuf.lex_buffer_len - lexbuf.lex_start_pos | 0) + n | 0) <= #lexbuf.lex_buffer) do
                  Bytes.blit(lexbuf.lex_buffer, lexbuf.lex_start_pos, lexbuf.lex_buffer, 0, lexbuf.lex_buffer_len - lexbuf.lex_start_pos | 0);
                end else do
                  var newlen = (#lexbuf.lex_buffer << 1);
                  if (((lexbuf.lex_buffer_len - lexbuf.lex_start_pos | 0) + n | 0) > newlen) do
                    throw [
                ...
                for(var i = 0 ,i_finish = #t - 1 | 0; i <= i_finish; ++i)do
