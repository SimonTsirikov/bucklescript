...
function f(match) do
  if (Caml_exceptions.caml_is_extension(match)) then do
    if (match == Caml_builtin_exceptions.not_found) then do
      return 0;
    end else if (match[1] == Caml_builtin_exceptions.invalid_argument or match == Caml_builtin_exceptions.stack_overflow) then do
      return 1;
    end else if (match[1] == Caml_builtin_exceptions.sys_error) then do
      return 2;
    end else do
      return ;
    end end  end  end 
...
function g(match) do
  if (Caml_exceptions.caml_is_extension(match)) then do
    if (match == Caml_builtin_exceptions.not_found or match[1] == Caml_builtin_exceptions.invalid_argument) then do
      return 0;
    end else if (match[1] == Caml_builtin_exceptions.sys_error) then do
      return 2;
    end else if (match[1] == A or match[1] == B) then do
      return match[2];
    end else do
      return ;
    end end  end  end 
