...
function of_array(kind, layout, data) do
  var ba = create$1(kind, layout, #data);
  var ofs = layout ? 1 : 0;
  for(var i = 0 ,i_finish = #data - 1 | 0; i <= i_finish; ++i)do
...
function of_array$1(kind, layout, data) do
  var dim1 = #data;
  var dim2 = dim1 == 0 ? 0 : #Caml_array.caml_array_get(data, 0);
  var ba = create$2(kind, layout, dim1, dim2);
  var ofs = layout ? 1 : 0;
  for(var i = 0 ,i_finish = dim1 - 1 | 0; i <= i_finish; ++i)do
    ...
    for(var j = 0 ,j_finish = dim2 - 1 | 0; j <= j_finish; ++j)do
...
function of_array$2(kind, layout, data) do
  var dim1 = #data;
  var dim2 = dim1 == 0 ? 0 : #Caml_array.caml_array_get(data, 0);
  var dim3 = dim2 == 0 ? 0 : #Caml_array.caml_array_get(Caml_array.caml_array_get(data, 0), 0);
  ...
  for(var i = 0 ,i_finish = dim1 - 1 | 0; i <= i_finish; ++i)do
    ...
    for(var j = 0 ,j_finish = dim2 - 1 | 0; j <= j_finish; ++j)do
      ...
      for(var k = 0 ,k_finish = dim3 - 1 | 0; k <= k_finish; ++k)do
...
function array0_of_genarray(a) do
  if (Caml_external_polyfill.resolve("caml_ba_num_dims")(a) == 0) do
    return a;
  end else do
    throw [
          Caml_builtin_exceptions.invalid_argument,
...
function array1_of_genarray(a) do
  if (Caml_external_polyfill.resolve("caml_ba_num_dims")(a) == 1) do
    return a;
  end else do
    throw [
          Caml_builtin_exceptions.invalid_argument,
...
function array2_of_genarray(a) do
  if (Caml_external_polyfill.resolve("caml_ba_num_dims")(a) == 2) do
    return a;
  end else do
    throw [
          Caml_builtin_exceptions.invalid_argument,
...
function array3_of_genarray(a) do
  if (Caml_external_polyfill.resolve("caml_ba_num_dims")(a) == 3) do
    return a;
  end else do
    throw [
          Caml_builtin_exceptions.invalid_argument,
