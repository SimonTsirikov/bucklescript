...
function set_method(table, label, element) do
  ...
  if (Belt_MapInt.getExn(table.methods_by_label, label)) do
    var array = table;
    var label$1 = label;
...
function new_cache(table) do
  ...
  var n$1 = n % 2 == 0 or n > (2 + ((Caml_array.caml_array_get(table.methods, 1) << 4) / 32 | 0) | 0) ? n : new_method(table);
