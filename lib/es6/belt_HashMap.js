...
function set0(h, key, value, eq, hash) do
  ...
  var l = h_buckets[i];
  if (l ~= undefined) do
    if (replaceInBucket(eq, key, value, l)) do
      h_buckets[i] = do
        key: key,
        value: value,
        next: l
  ...
  if (bucket ~= undefined) do
    ...
    if (eq(bucket.key, key)) do
      ...
      while(true) do
        ...
        var prec = _prec;
        if (bucket$1 ~= undefined) do
          var cell_next = bucket$1.next;
          if (eq$1(bucket$1.key, key$1)) do
            prec.next = cell_next;
