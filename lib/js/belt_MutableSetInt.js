...
Belt_SortArrayInt = require("./belt_SortArrayInt.js");
Belt_internalAVLset = require("./belt_internalAVLset.js");
Belt_internalSetInt = require("./belt_internalSetInt.js");
...
function add(d, k) do
  oldRoot = d.data;
  v = Belt_internalSetInt.addMutate(oldRoot, k);
...
function split(d, key) do
  ...
  i = Belt_SortArrayInt.binarySearch(arr, key);
  len = #arr;
  if (i < 0) then do
    next = (-i | 0) - 1 | 0;
    return --[ tuple ]--[
  ...
  if (dataa$1 ~= null) then do
    if (datab$1 ~= null) then do
      ...
      if (tmp[sizea - 1 | 0] < tmp[sizea] or tmp[totalSize - 1 | 0] < tmp[0]) then do
        ...
        k = Belt_SortArrayInt.intersect(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0);
        return do
  ...
  if (dataa$1 ~= null) then do
    if (datab$1 ~= null) then do
      ...
      if (tmp[sizea - 1 | 0] < tmp[sizea] or tmp[totalSize - 1 | 0] < tmp[0]) then do
        ...
        k = Belt_SortArrayInt.diff(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0);
        return do
                data: Belt_internalAVLset.fromSortedArrayAux(tmp2, 0, k)
              end;
      end end 
  ...
  if (dataa$1 ~= null) then do
    if (datab$1 ~= null) then do
      ...
      if (tmp[sizea - 1 | 0] < tmp[sizea]) then do
        ...
        k = Belt_SortArrayInt.union(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0);
        return do
                data: Belt_internalAVLset.fromSortedArrayAux(tmp2, 0, k)
              end;
      end end 
