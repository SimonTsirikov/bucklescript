  ...
  while(true) do
    ...
    if (lx_1) then do
      if (ly_1) then do
        _ly = ly_1[2];
        _lx = lx_1[2];
        _acc = List.rev_append(Curry._2(f, lx_1[1], ly_1[1]), acc);
        ::continue:: ;
      end else do
        error({
          Caml_builtin_exceptions.invalid_argument,
...
function flat_map_aux(f, _acc, append, _lx) do
  while(true) do
    ...
    if (lx) then do
      _lx = lx[2];
      _acc = List.rev_append(Curry._1(f, lx[1]), acc);
      ::continue:: ;
    end else do
...
function map2_last(f, l1, l2) do
  if (l1) then do
    ...
    if (l2) then do
      r = Curry._3(f, false, u, l2[1]);
      return --[[ :: ]]{
              r,
              map2_last(f, l1_1, l2[2])
            };
    end else do
      error({
        Caml_builtin_exceptions.invalid_argument,
...
function fold_right2_last(f, l1, l2, accu) do
  if (l1) then do
    ...
    if (not l1_1) then do
      if (l2) then do
        if (not l2[2]) then do
          return Curry._4(f, true, last1, l2[1], accu);
        end
         end 
      end else do
        error({
    ...
    if (l2) then do
      return Curry._4(f, false, last1, l2[1], fold_right2_last(f, l1_1, l2[2], accu));
    end else do
      error({
        Caml_builtin_exceptions.invalid_argument,
        "List.fold_right2"
...
function for_all2_no_exn(p, _l1, _l2) do
  while(true) do
    ...
    l1 = _l1;
    if (l1) then do
      if (l2 and Curry._2(p, l1[1], l2[1])) then do
        _l2 = l2[2];
        _l1 = l1[2];
        ::continue:: ;
      end else do
...
function reduce_from_left(fn, lst) do
  if (lst) then do
    return List.fold_left(fn, lst[1], lst[2]);
  end else do
    error({
      Caml_builtin_exceptions.invalid_argument,
