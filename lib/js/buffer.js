...
function sub(b, ofs, len) {
  if (ofs < 0 or len < 0 or ofs > (b.position - len | 0)) {
    throw [
...
function blit(src, srcoff, dst, dstoff, len) {
  if (len < 0 or srcoff < 0 or srcoff > (src.position - len | 0) or dstoff < 0 or dstoff > (dst.length - len | 0)) {
    throw [
          Caml_builtin_exceptions.invalid_argument,
          "Buffer.blit"
        ];
...
function add_substring(b, s, offset, len) {
  if (offset < 0 or len < 0 or offset > (s.length - len | 0)) {
    throw [
          Caml_builtin_exceptions.invalid_argument,
