...
v = __JSON.parse(" { \"x\" : [1, 2, 3 ] } ");

add_test("File \"js_json_test.ml\", line 23, characters 11-18", (function(param) do
        ty = Js_json.classify(v);
        if (type(ty) == "number" or ty.tag ~= --[[ JSONObject ]]2) then do
          return --[[ Ok ]]Block.__(4, {false});
        end else do
          match = Js_dict.get(ty[1], "x");
          if (match ~= nil) then do
            ty2 = Js_json.classify(Caml_option.valFromOption(match));
            if (type(ty2) == "number" or ty2.tag ~= --[[ JSONArray ]]3) then do
              return --[[ Ok ]]Block.__(4, {false});
            end else do
              ty2[1].forEach((function(x) do
                      ty3 = Js_json.classify(x);
                      if (type(ty3) == "number") then do
                        error({
...
json = __JSON.parse(__JSON.stringify(__null));
...
json_1 = __JSON.parse(__JSON.stringify("test string"));

ty_1 = Js_json.classify(json_1);

if (type(ty_1) == "number") then do
  ...
  eq("File \"js_json_test.ml\", line 64, characters 31-38", ty_1[1], "test string");
end end  end 

json_2 = __JSON.parse(__JSON.stringify(1.23456789));

ty_2 = Js_json.classify(json_2);

exit = 0;

if (type(ty_2) == "number" or ty_2.tag ~= --[[ JSONNumber ]]1) then do
  exit = 1;
end else do
  eq("File \"js_json_test.ml\", line 74, characters 31-38", ty_2[1], 1.23456789);
end end 
...
json_3 = __JSON.parse(__JSON.stringify(-1347440721));

ty_3 = Js_json.classify(json_3);

exit_1 = 0;

if (type(ty_3) == "number" or ty_3.tag ~= --[[ JSONNumber ]]1) then do
  exit_1 = 1;
end else do
  eq("File \"js_json_test.ml\", line 84, characters 31-38", ty_3[1] | 0, -1347440721);
end end 
...
function test(v) do
  json = __JSON.parse(__JSON.stringify(v));
  ty = Js_json.classify(json);
  if (type(ty) == "number") then do
...
json_4 = __JSON.parse(__JSON.stringify(dict));

ty_4 = Js_json.classify(json_4);

if (type(ty_4) == "number") then do
  ...
  end else if (ta[1] ~= "test string") then do
...
function eq_at_i(loc, json, i, kind, expected) do
  ...
  if (type(ty) == "number") then do
    ...
    ty_1 = Js_json.classify(Caml_array.caml_array_get(ty[1], i));
...
json_5 = __JSON.parse(__JSON.stringify(__Array.map((function(prim) do
                return prim;
              end end), {
              "string 0",
              "string 1",
...
json_6 = __JSON.parse(__JSON.stringify({
          "string 0",
          "string 1",
          "string 2"
        }));
...
json_7 = __JSON.parse(__JSON.stringify(a));
...
json_8 = __JSON.parse(__JSON.stringify(__Array.map((function(prim) do
                return prim;
              end end), a_1)));
...
json_9 = __JSON.parse(__JSON.stringify(a_2));
...
if (type(ty_6) == "number") then do
  ...
  if (type(ty_7) == "number") then do
    ...
    if (type(ty_8) == "number") then do
      ...
      eq("File \"js_json_test.ml\", line 277, characters 40-47", ty_8[1], "bbb");
    end end  end 
  end else do
    add_test("File \"js_json_test.ml\", line 280, characters 18-25", (function(param) do
            return --[[ Ok ]]Block.__(4, {false});
...
eq("File \"js_json_test.ml\", line 295, characters 12-19", __JSON.stringify({
          1,
          2,
          3
        }), "[1,2,3]");

eq("File \"js_json_test.ml\", line 299, characters 2-9", __JSON.stringify({
          foo = 1,
          bar = "hello",
          baz = {
            baaz = 10
...
eq("File \"js_json_test.ml\", line 303, characters 12-19", __JSON.stringify(nil), "null");

eq("File \"js_json_test.ml\", line 305, characters 12-19", __JSON.stringify(nil), nil);
...
eq("File \"js_json_test.ml\", line 314, characters 5-12", Js_json.decodeString(__null), nil);
...
eq("File \"js_json_test.ml\", line 328, characters 5-12", Js_json.decodeNumber(__null), nil);
...
eq("File \"js_json_test.ml\", line 342, characters 5-12", Js_json.decodeObject(__null), nil);
...
eq("File \"js_json_test.ml\", line 357, characters 5-12", Js_json.decodeArray(__null), nil);
...
eq("File \"js_json_test.ml\", line 371, characters 5-12", Js_json.decodeBoolean(__null), nil);
...
eq("File \"js_json_test.ml\", line 385, characters 5-12", Js_json.decodeNull(__null), nil);
