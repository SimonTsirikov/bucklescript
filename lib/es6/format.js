...
function pp_force_break_line(state) do
  ...
  if (match) do
    var match$1 = match[0];
    var width = match$1[1];
    if (width > state.pp_space_left and (match$1[0] - 1 >>> 0) <= 3) do
  ...
  if (typeof param == "number") do
    switch (param) do
      case --[ Pp_stab ]--0 :
          ...
          if (match) do
            ...
            var add_tab = function (n, ls) do
              if (ls) do
                var x = ls[0];
                if (Caml_obj.caml_lessthan(n, x)) do
                  return --[ :: ]--[
                          n,
                          ls
                        ];
      ...
      case --[ Pp_if_newline ]--4 :
          if (state.pp_current_indent ~= (state.pp_margin - state.pp_space_left | 0)) do
            var state$1 = state;
    ...
    switch (param.tag | 0) do
      ...
      case --[ Pp_break ]--1 :
          ...
          if (match$6) do
            ...
            switch (match$7[0]) do
              ...
              case --[ Pp_hovbox ]--3 :
                  if (size > state.pp_space_left) do
                    return break_new_line(state, off, width);
                  end else do
                    return break_same_line(state, n);
                  end
              case --[ Pp_box ]--4 :
                  if (state.pp_is_new_line or !(size > state.pp_space_left or state.pp_current_indent > ((state.pp_margin - width | 0) + off | 0))) do
                    return break_same_line(state, n);
                  end else do
                    return break_new_line(state, off, width);
                  end
          ...
          if (match$8) do
            ...
            var find = function (n, _param) do
              while(true) do
                ...
                if (param) do
                  var x = param[0];
                  if (Caml_obj.caml_greaterequal(x, n)) do
                    return x;
                  end else do
                    _param = param[1];
                    continue ;
      ...
      case --[ Pp_begin ]--3 :
          ...
          if (insertion_point$1 > state.pp_max_indent) do
...
function advance_left(state) do
  try do
    ...
    while(true) do
      ...
      if (size < 0 and (state$1.pp_right_total - state$1.pp_left_total | 0) < state$1.pp_space_left) do
        return 0;
      end else do
...
function set_size(state, ty) do
  ...
  if (match) do
    ...
    var t = match[1];
    if (match$1[0] < state.pp_left_total) do
...
function pp_open_box_gen(state, indent, br_ty) do
  ...
  end else if (state.pp_curr_depth == state.pp_max_boxes) do
    var state$1 = state;
...
function pp_close_tbox(state, param) do
  if (state.pp_curr_depth > 1 and state.pp_curr_depth < state.pp_max_boxes) do
    var elem = do
      elem_size: 0,
      token: --[ Pp_tend ]--2,
      length: 0
...
function output_formatting_lit(ppf, fmting_lit) do
  if (typeof fmting_lit == "number") do
    switch (fmting_lit) do
      case --[ Close_box ]--0 :
  ...
  if (typeof acc == "number") do
    ...
    switch (acc.tag | 0) do
      ...
      case --[ Acc_string_literal ]--2 :
          var p$3 = acc[0];
          if (typeof p$3 ~= "number" and !p$3.tag) do
            var match$2 = p$3[1];
            if (typeof match$2 ~= "number" and match$2.tag == --[ Magic_size ]--1) do
              p = p$3[0];
              size = match$2[1];
              s = acc[1];
              exit = 1;
      ...
      case --[ Acc_char_literal ]--3 :
          var p$4 = acc[0];
          if (typeof p$4 ~= "number" and !p$4.tag) do
            var match$3 = p$4[1];
            if (typeof match$3 ~= "number" and match$3.tag == --[ Magic_size ]--1) do
              p$1 = p$4[0];
              size$1 = match$3[1];
              c = acc[1];
              exit = 2;
      ...
      case --[ Acc_data_string ]--4 :
          var p$5 = acc[0];
          if (typeof p$5 ~= "number" and !p$5.tag) do
            var match$4 = p$5[1];
            if (typeof match$4 ~= "number" and match$4.tag == --[ Magic_size ]--1) do
              p = p$5[0];
              size = match$4[1];
              s = acc[1];
              exit = 1;
      ...
      case --[ Acc_data_char ]--5 :
          var p$6 = acc[0];
          if (typeof p$6 ~= "number" and !p$6.tag) do
            var match$5 = p$6[1];
            if (typeof match$5 ~= "number" and match$5.tag == --[ Magic_size ]--1) do
              p$1 = p$6[0];
              size$1 = match$5[1];
              c = acc[1];
              exit = 2;
...
function strput_acc(ppf, acc) do
  ...
  if (typeof acc == "number") do
    ...
    switch (acc.tag | 0) do
      ...
      case --[ Acc_delay ]--6 :
          var p$7 = acc[0];
          if (typeof p$7 ~= "number" and !p$7.tag) do
            var match$6 = p$7[1];
            if (typeof match$6 ~= "number" and match$6.tag == --[ Magic_size ]--1) do
