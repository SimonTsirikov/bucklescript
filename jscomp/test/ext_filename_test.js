...
function path_as_directory(x) do
  if (x == "" or Ext_string_test.ends_with(x, Filename.dir_sep)) do
    return x;
  end else do
...
function relative_path(file_or_dir_1, file_or_dir_2) do
  ...
  var go = function (_dir1, _dir2) do
    while(true) do
      var dir2 = _dir2;
      var dir1 = _dir1;
      if (dir1 and dir2 and dir1[0] == dir2[0]) do
        _dir2 = dir2[1];
        _dir1 = dir1[1];
        continue ;
      end
...
function node_relative_path(node_modules_shorten, file1, dep_file) do
  ...
  var len = #file2;
  if (node_modules_shorten and v >= 0) do
    var skip = function (_i) do
      while(true) do
        ...
        if (i >= len) do
          ...
          if (curr_char == os_path_separator_char or curr_char == --[ "." ]--46) do
            _i = i + 1 | 0;
            continue ;
          end else do
            return i;
...
function find_root_filename(_cwd, filename) do
  while(true) do
    var cwd = _cwd;
    if (Caml_external_polyfill.resolve("caml_sys_file_exists")(Filename.concat(cwd, filename))) do
      return cwd;
    end else do
...
function combine(p1, p2) do
  if (p1 == "" or p1 == Filename.current_dir_name) do
    return p2;
  end else if (p2 == "" or p2 == Filename.current_dir_name) do
    return p1;
  end else if (Curry._1(Filename.is_relative, p2)) do
...
function split_aux(p) do
  ...
  while(true) do
    ...
    if (dir == p$1) do
      ...
      if (new_path == Filename.dir_sep) do
        _p = dir;
        continue ;
      end else do
        _acc = --[ :: ]--[
  ...
  var normalize_list = function (_acc, _paths) do
    while(true) do
      ...
      if (paths) do
        ...
        var x = paths[0];
        _paths = xs;
        if (x == Ext_string_test.current_dir_lit) do
          continue ;
        end else if (x == Ext_string_test.parent_dir_lit) do
...
end else if (Sys.win32 or false) do
