...
function parse_and_expand_argv_dynamic_aux(allow_expand, current, argv, speclist, anonfun, errmsg) {
  ...
  var convert_error = function (error) {
    ...
    if (Caml_obj.caml_equal(error, --[ Unknown ]--Block.__(0, ["-help"])) or Caml_obj.caml_equal(error, --[ Unknown ]--Block.__(0, ["--help"]))) {
      return [
              Help,
              $$Buffer.contents(b)
            ];
  ...
  while(current.contents < #argv.contents) {
    try {
      var s = Caml_array.caml_array_get(argv.contents, current.contents);
      if (#s >= 1 and Caml_string.get(s, 0) == --[ "-" ]--45) {
        var match;
        try {
          match = --[ tuple ]--[
        ...
        return function get_arg(param) {
          if (follow ~= undefined) {
            return follow;
          } else if ((current.contents + 1 | 0) < #argv.contents) {
        ...
        return function treat_action(param) {
          switch (param.tag | 0) {
            ...
            case --[ Rest ]--12 :
                var f = param[0];
                while(current.contents < (#argv.contents - 1 | 0)) {
...
function trim_cr(s) {
  var len = #s;
  if (len > 0 and Caml_string.get(s, len - 1 | 0) == --[ "\r" ]--13) {
