

import * as Obj from "./obj.js";
import * as Block from "./block.js";
import * as Curry from "./curry.js";
import * as $$Buffer from "./buffer.js";
import * as Printf from "./printf.js";
import * as Caml_io from "./caml_io.js";
import * as Caml_array from "./caml_array.js";
import * as Pervasives from "./pervasives.js";
import * as Caml_js_exceptions from "./caml_js_exceptions.js";
import * as Caml_external_polyfill from "./caml_external_polyfill.js";
import * as Caml_builtin_exceptions from "./caml_builtin_exceptions.js";

var printers = do
  contents: --[ [] ]--0
end;

var locfmt = --[ Format ]--[
  --[ String_literal ]--Block.__(11, [
      "File \"",
      --[ String ]--Block.__(2, [
          --[ No_padding ]--0,
          --[ String_literal ]--Block.__(11, [
              "\", line ",
              --[ Int ]--Block.__(4, [
                  --[ Int_d ]--0,
                  --[ No_padding ]--0,
                  --[ No_precision ]--0,
                  --[ String_literal ]--Block.__(11, [
                      ", characters ",
                      --[ Int ]--Block.__(4, [
                          --[ Int_d ]--0,
                          --[ No_padding ]--0,
                          --[ No_precision ]--0,
                          --[ Char_literal ]--Block.__(12, [
                              --[ "-" ]--45,
                              --[ Int ]--Block.__(4, [
                                  --[ Int_d ]--0,
                                  --[ No_padding ]--0,
                                  --[ No_precision ]--0,
                                  --[ String_literal ]--Block.__(11, [
                                      ": ",
                                      --[ String ]--Block.__(2, [
                                          --[ No_padding ]--0,
                                          --[ End_of_format ]--0
                                        ])
                                    ])
                                ])
                            ])
                        ])
                    ])
                ])
            ])
        ])
    ]),
  "File \"%s\", line %d, characters %d-%d: %s"
];

function field(x, i) do
  var f = x[i];
  if (typeof f == "number") do
    return Curry._1(Printf.sprintf(--[ Format ]--[
                    --[ Int ]--Block.__(4, [
                        --[ Int_d ]--0,
                        --[ No_padding ]--0,
                        --[ No_precision ]--0,
                        --[ End_of_format ]--0
                      ]),
                    "%d"
                  ]), f);
  end else if ((f.tag | 0) == Obj.string_tag) do
    return Curry._1(Printf.sprintf(--[ Format ]--[
                    --[ Caml_string ]--Block.__(3, [
                        --[ No_padding ]--0,
                        --[ End_of_format ]--0
                      ]),
                    "%S"
                  ]), f);
  end else if ((f.tag | 0) == Obj.double_tag) do
    return Pervasives.string_of_float(f);
  end else do
    return "_";
  end
end

function other_fields(x, i) do
  if (i >= #x) do
    return "";
  end else do
    return Curry._2(Printf.sprintf(--[ Format ]--[
                    --[ String_literal ]--Block.__(11, [
                        ", ",
                        --[ String ]--Block.__(2, [
                            --[ No_padding ]--0,
                            --[ String ]--Block.__(2, [
                                --[ No_padding ]--0,
                                --[ End_of_format ]--0
                              ])
                          ])
                      ]),
                    ", %s%s"
                  ]), field(x, i), other_fields(x, i + 1 | 0));
  end
end

function fields(x) do
  var match = #x;
  switch (match) do
    case 0 :
    case 1 :
        return "";
    case 2 :
        return Curry._1(Printf.sprintf(--[ Format ]--[
                        --[ Char_literal ]--Block.__(12, [
                            --[ "(" ]--40,
                            --[ String ]--Block.__(2, [
                                --[ No_padding ]--0,
                                --[ Char_literal ]--Block.__(12, [
                                    --[ ")" ]--41,
                                    --[ End_of_format ]--0
                                  ])
                              ])
                          ]),
                        "(%s)"
                      ]), field(x, 1));
    default:
      return Curry._2(Printf.sprintf(--[ Format ]--[
                      --[ Char_literal ]--Block.__(12, [
                          --[ "(" ]--40,
                          --[ String ]--Block.__(2, [
                              --[ No_padding ]--0,
                              --[ String ]--Block.__(2, [
                                  --[ No_padding ]--0,
                                  --[ Char_literal ]--Block.__(12, [
                                      --[ ")" ]--41,
                                      --[ End_of_format ]--0
                                    ])
                                ])
                            ])
                        ]),
                      "(%s%s)"
                    ]), field(x, 1), other_fields(x, 2));
  end
end

function to_string(x) do
  var _param = printers.contents;
  while(true) do
    var param = _param;
    if (param) do
      var match;
      try do
        match = Curry._1(param[0], x);
      end
      catch (exn)do
        match = undefined;
      end
      if (match ~= undefined) do
        return match;
      end else do
        _param = param[1];
        continue ;
      end
    end else if (x == Caml_builtin_exceptions.out_of_memory) do
      return "Out of memory";
    end else if (x == Caml_builtin_exceptions.stack_overflow) do
      return "Stack overflow";
    end else if (x[0] == Caml_builtin_exceptions.match_failure) do
      var match$1 = x[1];
      var $$char = match$1[2];
      return Curry._5(Printf.sprintf(locfmt), match$1[0], match$1[1], $$char, $$char + 5 | 0, "Pattern matching failed");
    end else if (x[0] == Caml_builtin_exceptions.assert_failure) do
      var match$2 = x[1];
      var $$char$1 = match$2[2];
      return Curry._5(Printf.sprintf(locfmt), match$2[0], match$2[1], $$char$1, $$char$1 + 6 | 0, "Assertion failed");
    end else if (x[0] == Caml_builtin_exceptions.undefined_recursive_module) do
      var match$3 = x[1];
      var $$char$2 = match$3[2];
      return Curry._5(Printf.sprintf(locfmt), match$3[0], match$3[1], $$char$2, $$char$2 + 6 | 0, "Undefined recursive module");
    end else if ((x.tag | 0) ~= 0) do
      return x[0];
    end else do
      var constructor = x[0][0];
      return constructor .. fields(x);
    end
  end;
end

function print(fct, arg) do
  try do
    return Curry._1(fct, arg);
  end
  catch (raw_x)do
    var x = Caml_js_exceptions.internalToOCamlException(raw_x);
    Curry._1(Printf.eprintf(--[ Format ]--[
              --[ String_literal ]--Block.__(11, [
                  "Uncaught exception: ",
                  --[ String ]--Block.__(2, [
                      --[ No_padding ]--0,
                      --[ Char_literal ]--Block.__(12, [
                          --[ "\n" ]--10,
                          --[ End_of_format ]--0
                        ])
                    ])
                ]),
              "Uncaught exception: %s\n"
            ]), to_string(x));
    Caml_io.caml_ml_flush(Pervasives.stderr);
    throw x;
  end
end

function $$catch(fct, arg) do
  try do
    return Curry._1(fct, arg);
  end
  catch (raw_x)do
    var x = Caml_js_exceptions.internalToOCamlException(raw_x);
    Caml_io.caml_ml_flush(Pervasives.stdout);
    Curry._1(Printf.eprintf(--[ Format ]--[
              --[ String_literal ]--Block.__(11, [
                  "Uncaught exception: ",
                  --[ String ]--Block.__(2, [
                      --[ No_padding ]--0,
                      --[ Char_literal ]--Block.__(12, [
                          --[ "\n" ]--10,
                          --[ End_of_format ]--0
                        ])
                    ])
                ]),
              "Uncaught exception: %s\n"
            ]), to_string(x));
    return Pervasives.exit(2);
  end
end

function convert_raw_backtrace_slot(param) do
  throw [
        Caml_builtin_exceptions.failure,
        "convert_raw_backtrace_slot not implemented"
      ];
end

function convert_raw_backtrace(bt) do
  try do
    return --[ () ]--0;
  end
  catch (raw_exn)do
    var exn = Caml_js_exceptions.internalToOCamlException(raw_exn);
    if (exn[0] == Caml_builtin_exceptions.failure) do
      return ;
    end else do
      throw exn;
    end
  end
end

function format_backtrace_slot(pos, slot) do
  var info = function (is_raise) do
    if (is_raise) do
      if (pos == 0) do
        return "Raised at";
      end else do
        return "Re-raised at";
      end
    end else if (pos == 0) do
      return "Raised by primitive operation at";
    end else do
      return "Called from";
    end
  end;
  if (slot.tag) do
    if (slot[--[ is_raise ]--0]) do
      return ;
    end else do
      return Curry._1(Printf.sprintf(--[ Format ]--[
                      --[ String ]--Block.__(2, [
                          --[ No_padding ]--0,
                          --[ String_literal ]--Block.__(11, [
                              " unknown location",
                              --[ End_of_format ]--0
                            ])
                        ]),
                      "%s unknown location"
                    ]), info(false));
    end
  end else do
    return Curry._6(Printf.sprintf(--[ Format ]--[
                    --[ String ]--Block.__(2, [
                        --[ No_padding ]--0,
                        --[ String_literal ]--Block.__(11, [
                            " file \"",
                            --[ String ]--Block.__(2, [
                                --[ No_padding ]--0,
                                --[ Char_literal ]--Block.__(12, [
                                    --[ "\"" ]--34,
                                    --[ String ]--Block.__(2, [
                                        --[ No_padding ]--0,
                                        --[ String_literal ]--Block.__(11, [
                                            ", line ",
                                            --[ Int ]--Block.__(4, [
                                                --[ Int_d ]--0,
                                                --[ No_padding ]--0,
                                                --[ No_precision ]--0,
                                                --[ String_literal ]--Block.__(11, [
                                                    ", characters ",
                                                    --[ Int ]--Block.__(4, [
                                                        --[ Int_d ]--0,
                                                        --[ No_padding ]--0,
                                                        --[ No_precision ]--0,
                                                        --[ Char_literal ]--Block.__(12, [
                                                            --[ "-" ]--45,
                                                            --[ Int ]--Block.__(4, [
                                                                --[ Int_d ]--0,
                                                                --[ No_padding ]--0,
                                                                --[ No_precision ]--0,
                                                                --[ End_of_format ]--0
                                                              ])
                                                          ])
                                                      ])
                                                  ])
                                              ])
                                          ])
                                      ])
                                  ])
                              ])
                          ])
                      ]),
                    "%s file \"%s\"%s, line %d, characters %d-%d"
                  ]), info(slot[--[ is_raise ]--0]), slot[--[ filename ]--1], slot[--[ is_inline ]--5] ? " (inlined)" : "", slot[--[ line_number ]--2], slot[--[ start_char ]--3], slot[--[ end_char ]--4]);
  end
end

function print_raw_backtrace(outchan, raw_backtrace) do
  var outchan$1 = outchan;
  var backtrace = convert_raw_backtrace(raw_backtrace);
  if (backtrace ~= undefined) do
    var a = backtrace;
    for(var i = 0 ,i_finish = #a - 1 | 0; i <= i_finish; ++i)do
      var match = format_backtrace_slot(i, Caml_array.caml_array_get(a, i));
      if (match ~= undefined) do
        Curry._1(Printf.fprintf(outchan$1, --[ Format ]--[
                  --[ String ]--Block.__(2, [
                      --[ No_padding ]--0,
                      --[ Char_literal ]--Block.__(12, [
                          --[ "\n" ]--10,
                          --[ End_of_format ]--0
                        ])
                    ]),
                  "%s\n"
                ]), match);
      end
      
    end
    return --[ () ]--0;
  end else do
    return Printf.fprintf(outchan$1, --[ Format ]--[
                --[ String_literal ]--Block.__(11, [
                    "(Program not linked with -g, cannot print stack backtrace)\n",
                    --[ End_of_format ]--0
                  ]),
                "(Program not linked with -g, cannot print stack backtrace)\n"
              ]);
  end
end

function print_backtrace(outchan) do
  return print_raw_backtrace(outchan, --[ () ]--0);
end

function raw_backtrace_to_string(raw_backtrace) do
  var backtrace = convert_raw_backtrace(raw_backtrace);
  if (backtrace ~= undefined) do
    var a = backtrace;
    var b = $$Buffer.create(1024);
    for(var i = 0 ,i_finish = #a - 1 | 0; i <= i_finish; ++i)do
      var match = format_backtrace_slot(i, Caml_array.caml_array_get(a, i));
      if (match ~= undefined) do
        Curry._1(Printf.bprintf(b, --[ Format ]--[
                  --[ String ]--Block.__(2, [
                      --[ No_padding ]--0,
                      --[ Char_literal ]--Block.__(12, [
                          --[ "\n" ]--10,
                          --[ End_of_format ]--0
                        ])
                    ]),
                  "%s\n"
                ]), match);
      end
      
    end
    return $$Buffer.contents(b);
  end else do
    return "(Program not linked with -g, cannot print stack backtrace)\n";
  end
end

function backtrace_slot_is_raise(param) do
  return param[--[ is_raise ]--0];
end

function backtrace_slot_is_inline(param) do
  if (param.tag) do
    return false;
  end else do
    return param[--[ is_inline ]--5];
  end
end

function backtrace_slot_location(param) do
  if (param.tag) do
    return ;
  end else do
    return do
            filename: param[--[ filename ]--1],
            line_number: param[--[ line_number ]--2],
            start_char: param[--[ start_char ]--3],
            end_char: param[--[ end_char ]--4]
          end;
  end
end

function backtrace_slots(raw_backtrace) do
  var match = convert_raw_backtrace(raw_backtrace);
  if (match ~= undefined) do
    var backtrace = match;
    var usable_slot = function (param) do
      if (param.tag) do
        return false;
      end else do
        return true;
      end
    end;
    var exists_usable = function (_i) do
      while(true) do
        var i = _i;
        if (i ~= -1) do
          if (usable_slot(Caml_array.caml_array_get(backtrace, i))) do
            return true;
          end else do
            _i = i - 1 | 0;
            continue ;
          end
        end else do
          return false;
        end
      end;
    end;
    if (exists_usable(#backtrace - 1 | 0)) do
      return backtrace;
    end else do
      return ;
    end
  end
  
end

function get_backtrace(param) do
  return raw_backtrace_to_string(--[ () ]--0);
end

function register_printer(fn) do
  printers.contents = --[ :: ]--[
    fn,
    printers.contents
  ];
  return --[ () ]--0;
end

function exn_slot(x) do
  if (x.tag) do
    return x;
  end else do
    return x[0];
  end
end

function exn_slot_id(x) do
  var slot = exn_slot(x);
  return slot[1];
end

function exn_slot_name(x) do
  var slot = exn_slot(x);
  return slot[0];
end

var uncaught_exception_handler = do
  contents: undefined
end;

function set_uncaught_exception_handler(fn) do
  uncaught_exception_handler.contents = fn;
  return --[ () ]--0;
end

function record_backtrace(prim) do
  return --[ () ]--0;
end

function backtrace_status(prim) do
  return --[ () ]--0;
end

function get_raw_backtrace(prim) do
  return --[ () ]--0;
end

function get_callstack(prim) do
  return --[ () ]--0;
end

var Slot = do
  is_raise: backtrace_slot_is_raise,
  is_inline: backtrace_slot_is_inline,
  $$location: backtrace_slot_location,
  format: format_backtrace_slot
end;

function raw_backtrace_length(prim) do
  return Caml_external_polyfill.resolve("caml_raw_backtrace_length")(prim);
end

function get_raw_backtrace_slot(prim, prim$1) do
  return Caml_external_polyfill.resolve("caml_raw_backtrace_slot")(prim, prim$1);
end

function get_raw_backtrace_next_slot(prim) do
  return Caml_external_polyfill.resolve("caml_raw_backtrace_next_slot")(prim);
end

export do
  to_string ,
  print ,
  $$catch ,
  print_backtrace ,
  get_backtrace ,
  record_backtrace ,
  backtrace_status ,
  register_printer ,
  get_raw_backtrace ,
  print_raw_backtrace ,
  raw_backtrace_to_string ,
  get_callstack ,
  set_uncaught_exception_handler ,
  backtrace_slots ,
  Slot ,
  raw_backtrace_length ,
  get_raw_backtrace_slot ,
  convert_raw_backtrace_slot ,
  get_raw_backtrace_next_slot ,
  exn_slot_id ,
  exn_slot_name ,
  
end
--[ No side effect ]--
