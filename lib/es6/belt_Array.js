...
function reverseInPlace(xs) do
  ...
  var ofs = 0;
  var len$1 = len;
  for(var i = 0 ,i_finish = (len$1 / 2 | 0) - 1 | 0; i <= i_finish; ++i)do
...
function rangeBy(start, finish, step) do
  var cut = finish - start | 0;
  if (cut < 0 or step <= 0) do
    return [];
  end else do
    var nb = (cut / step | 0) + 1 | 0;
    var arr = new Array(nb);
    var cur = start;
    for(var i = 0 ,i_finish = nb - 1 | 0; i <= i_finish; ++i)do
      arr[i] = cur;
      cur = cur + step | 0;
    end
    return arr;
...
function concat(a1, a2) do
  ...
  for(var i = 0 ,i_finish = l1 - 1 | 0; i <= i_finish; ++i)do
    a1a2[i] = a1[i];
  end
  for(var i$1 = 0 ,i_finish$1 = l2 - 1 | 0; i$1 <= i_finish$1; ++i$1)do
...
function concatMany(arrs) do
  var lenArrs = #arrs;
  var totalLen = 0;
  for(var i = 0 ,i_finish = lenArrs - 1 | 0; i <= i_finish; ++i)do
    totalLen = totalLen + #arrs[i] | 0;
  end
  var result = new Array(totalLen);
  totalLen = 0;
  for(var j = 0 ,j_finish = lenArrs - 1 | 0; j <= j_finish; ++j)do
    var cur = arrs[j];
    for(var k = 0 ,k_finish = #cur - 1 | 0; k <= k_finish; ++k)do
...
function slice(a, offset, len) do
  if (len <= 0) do
    ...
    if (copyLength <= 0) do
      ...
      for(var i = 0 ,i_finish = copyLength - 1 | 0; i <= i_finish; ++i)do
        result[i] = a[ofs + i | 0];
      end
      return result;
    end
...
function fill(a, offset, len, v) do
  if (len > 0) do
    ...
    if (fillLength > 0) do
      for(var i = ofs ,i_finish = (ofs + fillLength | 0) - 1 | 0; i <= i_finish; ++i)do
        a[i] = v;
      end
      return --[ () ]--0;
    end else do
...
function blitUnsafe(a1, srcofs1, a2, srcofs2, blitLength) do
  if (srcofs2 <= srcofs1) do
    for(var j = 0 ,j_finish = blitLength - 1 | 0; j <= j_finish; ++j)do
      a2[j + srcofs2 | 0] = a1[j + srcofs1 | 0];
    end
    return --[ () ]--0;
  end else do
    for(var j$1 = blitLength - 1 | 0; j$1 >= 0; --j$1)do
...
function everyAux2(arr1, arr2, _i, b, len) do
  while(true) do
    ...
    if (i == len) do
      return true;
    end else if (b(arr1[i], arr2[i])) do
      _i = i + 1 | 0;
      continue ;
    end else do
      return false;
...
function some2U(a, b, p) do
  ...
  while(true) do
    ...
    if (i == len) do
      return false;
    end else if (b$1(arr1[i], arr2[i])) do
      return true;
    end else do
      _i = i + 1 | 0;
      continue ;
...
function partitionU(a, f) do
  ...
  var a1 = new Array(l);
  var a2 = new Array(l);
  for(var ii = 0 ,ii_finish = l - 1 | 0; ii <= ii_finish; ++ii)do
    var v = a[ii];
    if (f(v)) do
      a1[i] = v;
      i = i + 1 | 0;
