...
function bprint_char_set(buf, char_set) {
    ...
    while(true) {
      ...
      if (i < 256) {
        if (is_in_char_set(set, Pervasives.char_of_int(i))) {
          ...
          } else if (switcher > 47 or switcher < 1) {
  ...
  var print_second = function (set, i) {
    if (is_in_char_set(set, Pervasives.char_of_int(i))) {
      ...
      } else if ((switcher > 47 or switcher < 1) and !is_in_char_set(set, Pervasives.char_of_int(i + 1 | 0))) {
      ...
      if (is_in_char_set(set, Pervasives.char_of_int(i + 1 | 0))) {
        ...
        while(true) {
          var j = _j;
          if (j == 256 or !is_in_char_set(set$1, Pervasives.char_of_int(j))) {
            print_char(buf, i$1);
            print_char(buf, --[ "-" ]--45);
...
function fmtty_of_padding_fmtty(pad, fmtty) {
  if (typeof pad == "number" or !pad.tag) {
    return fmtty;
  } else {
...
function fmtty_of_precision_fmtty(prec, fmtty) {
  if (typeof prec == "number" and prec ~= 0) {
    return --[ Int_ty ]--Block.__(2, [fmtty]);
  } else {
    return fmtty;
  }
  ...
  if (width$1 <= len) {
    ...
    switch (padty$1) {
      ...
      case --[ Right ]--1 :
          ...
          break;
      case --[ Zeros ]--2 :
          if (len > 0 and (Caml_string.get(str, 0) == --[ "+" ]--43 or Caml_string.get(str, 0) == --[ "-" ]--45 or Caml_string.get(str, 0) == --[ " " ]--32)) {
            res[0] = Caml_string.get(str, 0);
            $$String.blit(str, 1, res, (width$1 - len | 0) + 1 | 0, len - 1 | 0);
          } else if (len > 1 and Caml_string.get(str, 0) == --[ "0" ]--48 and (Caml_string.get(str, 1) == --[ "x" ]--120 or Caml_string.get(str, 1) == --[ "X" ]--88)) {
    ...
    if (c >= 43) {
      switch (c - 43 | 0) {
        ...
        case 4 :
            return str;
        case 5 :
            if ((prec$1 + 2 | 0) > len and len > 1 and (Caml_string.get(str, 1) == --[ "x" ]--120 or Caml_string.get(str, 1) == --[ "X" ]--88)) {
      ...
      var is_valid = function (_i) {
        while(true) {
          ...
          if (i == len) {
            ...
            } else if (switcher > 22 or switcher < 1) {
              return true;
            } else {
              _i = i + 1 | 0;
              continue ;
  ...
  while(true) {
    ...
    if (typeof fmt == "number") {
      ...
      switch (fmt.tag | 0) {
        case --[ String ]--2 :
            var tmp = fmt[0];
            if (typeof tmp ~= "number" and tmp.tag) {
        ...
        case --[ Caml_string ]--3 :
            var tmp$1 = fmt[0];
            if (typeof tmp$1 ~= "number" and tmp$1.tag) {
        ...
        case --[ Bool ]--9 :
            var tmp$2 = fmt[0];
            if (typeof tmp$2 ~= "number" and tmp$2.tag) {
...
function fmt_ebb_of_string(legacy_behavior, str) {
      ...
      if (c >= 65) {
        ...
        if (c >= 32) {
          switch (c - 32 | 0) {
            ...
            case 5 :
                if ((str_ind + 1 | 0) < end_ind and Caml_string.get(str, str_ind + 1 | 0) == --[ "%" ]--37) {
            ...
            case 27 :
                ...
                var match$7;
                try {
                  if (str_ind$1 == end_ind$1 or Caml_string.get(str, str_ind$1) ~= --[ "<" ]--60) {
                  ...
                  if (exit == 1) {
                    ...
                    } else if (switcher == 2 or switcher == 1) {
                      throw Caml_builtin_exceptions.not_found;
                    } else {
                  ...
                  var exn = Caml_js_exceptions.internalToOCamlException(raw_exn);
                  if (exn == Caml_builtin_exceptions.not_found or exn[0] == Caml_builtin_exceptions.failure) {
                    match$7 = --[ tuple ]--[
                      str_ind$1,
                      --[ Break ]--Block.__(0, [
                          "@;",
            ...
            case 28 :
                ...
                try {
                  ...
                  if (exn$1 == Caml_builtin_exceptions.not_found or exn$1[0] == Caml_builtin_exceptions.failure) {
                    match$13 = undefined;
                  } else {
                    throw exn$1;
                  }
  ...
  var parse_flags = function (pct_ind, str_ind, end_ind, ign) {
    ...
    var set_flag = function (str_ind, flag) {
      if (flag.contents and !legacy_behavior$1) {
  ...
  var check_open_box = function (fmt) {
    if (typeof fmt == "number" or !(fmt.tag == --[ String_literal ]--11 and typeof fmt[1] == "number")) {
      return --[ () ]--0;
    } else {
      try {
  ...
  var parse_conversion = function (pct_ind, str_ind, end_ind, plus, hash, space, ign, pad, prec, padprec, symb) {
    ...
    switch (exit$2) {
      case 7 :
          ...
          break;
      case 8 :
          if (str_ind == end_ind or !is_int_base(Caml_string.get(str, str_ind))) {
    ...
    if (!legacy_behavior$1) {
      if (!plus_used and plus) {
        incompatible_flag(pct_ind, str_ind, symb, "'+'");
      }
      if (!hash_used and hash) {
        incompatible_flag(pct_ind, str_ind, symb, "'#'");
      }
      if (!space_used and space) {
        incompatible_flag(pct_ind, str_ind, symb, "' '");
      }
      if (!pad_used.contents and Caml_obj.caml_notequal(--[ Padding_EBB ]--[pad], --[ Padding_EBB ]--[--[ No_padding ]--0])) {
        incompatible_flag(pct_ind, str_ind, symb, "`padding'");
      }
      if (!prec_used.contents and Caml_obj.caml_notequal(--[ Precision_EBB ]--[prec], --[ Precision_EBB ]--[--[ No_precision ]--0])) {
    ...
    if (!ign_used.contents and ign) {
      var exit$3 = 0;
      if (symb >= 38) {
        if (symb ~= 44) {
          if (symb ~= 64 or !legacy_behavior$1) {
            exit$3 = 1;
          }
        ...
        if (!(symb >= 37 and legacy_behavior$1)) {
          exit$3 = 1;
        }
  ...
  var parse_after_padding = function (pct_ind, str_ind, end_ind, minus, plus, hash, space, ign, pad) {
    ...
    if (symb ~= 46) {
      ...
      if (exit == 2 and legacy_behavior$1) {
        ...
        if (c$prime >= 46) {
          ...
          if (c$prime >= 45) {
            ...
            if (c$prime$1 ~= 37) {
              ...
              if (c$prime$2 ~= 37 and c$prime$2 ~= 64) {
