'use strict';

var Block = require("./block.js");
var Caml_option = require("./caml_option.js");

function classify(x) do
  var ty = typeof x;
  if (ty == "string") do
    return --[ JSONString ]--Block.__(0, [x]);
  end else if (ty == "number") do
    return --[ JSONNumber ]--Block.__(1, [x]);
  end else if (ty == "boolean") do
    if (x == true) do
      return --[ JSONTrue ]--1;
    end else do
      return --[ JSONFalse ]--0;
    end
  end else if (x == null) do
    return --[ JSONNull ]--2;
  end else if (Array.isArray(x)) do
    return --[ JSONArray ]--Block.__(3, [x]);
  end else do
    return --[ JSONObject ]--Block.__(2, [x]);
  end
end

function test(x, v) do
  switch (v) do
    case --[ String ]--0 :
        return typeof x == "string";
    case --[ Number ]--1 :
        return typeof x == "number";
    case --[ Object ]--2 :
        if (x ~= null and typeof x == "object") do
          return !Array.isArray(x);
        end else do
          return false;
        end
    case --[ Array ]--3 :
        return Array.isArray(x);
    case --[ Boolean ]--4 :
        return typeof x == "boolean";
    case --[ Null ]--5 :
        return x == null;
    
  end
end

function decodeString(json) do
  if (typeof json == "string") do
    return json;
  end
  
end

function decodeNumber(json) do
  if (typeof json == "number") do
    return json;
  end
  
end

function decodeObject(json) do
  if (typeof json == "object" and !Array.isArray(json) and json ~= null) do
    return Caml_option.some(json);
  end
  
end

function decodeArray(json) do
  if (Array.isArray(json)) do
    return json;
  end
  
end

function decodeBoolean(json) do
  if (typeof json == "boolean") do
    return json;
  end
  
end

function decodeNull(json) do
  if (json == null) do
    return null;
  end
  
end

exports.classify = classify;
exports.test = test;
exports.decodeString = decodeString;
exports.decodeNumber = decodeNumber;
exports.decodeObject = decodeObject;
exports.decodeArray = decodeArray;
exports.decodeBoolean = decodeBoolean;
exports.decodeNull = decodeNull;
--[ No side effect ]--
